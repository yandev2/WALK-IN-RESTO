...
Langkah pertama yang wajib dilakukan sebelum kasir mulai melayani transaksi pelanggan adalah membuka shift kasir. Fungsi shift ini sangat penting untuk memastikan setiap uang masuk dan uang keluar tercatat rapi sesuai dengan penanggung jawab kasir pada jam kerja tersebut.

Untuk memulainya, klik menu Order Kasir pada bilah navigasi sebelah kiri. Jika shift belum dibuka, sistem akan menampilkan lencana peringatan berwarna kuning di bagian atas bertuliskan Shift Belum Dibuka. Silakan klik tombol tersebut untuk membuka jendela pop-up Buka Shift Kasir Baru.

Di formulir ini, kasir diminta memasukkan modal awal. Apa itu modal awal? Modal awal adalah jumlah uang tunai fisik yang sudah disiapkan di dalam laci kasir khusus untuk keperluan uang kembalian pelanggan.

Sebagai contoh, kita masukkan modal awal sebesar seratus ribu rupiah. Anda bisa mengetiknya secara manual, atau memanfaatkan tombol nominal cepat yang tersedia seperti lima puluh ribu, seratus ribu, hingga dua ratus ribu rupiah.

Jika ada catatan penting, misalnya uang pecahan dua ribuan sebanyak dua puluh lembar, Anda bisa menuliskannya di kolom catatan shift. Setelah semuanya sesuai, klik tombol Mulai Shift Kasir.

Perhatikan layar Anda sekarang. Lencana kuning tadi otomatis berubah menjadi lencana hijau aktif yang menampilkan nama kasir yang sedang bertugas, nominal modal awal, serta estimasi saldo uang tunai yang ada di laci secara real-time. Shift resmi berjalan, dan kita siap menerima pesanan pertama!
...
Sekarang, mari kita pilih menu yang dipesan pelanggan di panel sebelah kiri. Di sini tersaji katalog menu restoran Anda secara visual lengkap dengan gambar hidangan yang menggugah selera, nama menu, harga satuan, dan status ketersediaan.

Untuk mempercepat pencarian menu saat jam ramai, kasir bisa memfilter menu berdasarkan kategori makanan dan minuman, atau cukup mengetikkan nama menu di kolom pencarian di bagian atas.

Untuk menu simpel tanpa pilihan tambahan, kasir cukup menekan tombol Tambah. Jumlah porsi bisa langsung disesuaikan dengan menekan tombol plus atau minus pada kartu menu. Pesanan akan seketika masuk ke keranjang di sisi kanan.

Lalu, bagaimana jika pelanggan memesan menu yang memiliki varian atau permintaan khusus? Sangat mudah. Cukup klik tombol Pilih Opsi pada menu tersebut. Jendela pop-up kustomisasi akan terbuka.
...
Opsi kedua adalah Pembayaran Non-Tunai Kris. Cukup klik tab QRIS. Seketika, gambar kode Kris resmi outlet Anda akan tampil langsung di layar. Sistem juga menyertakan kode unik verifikasi pembayaran. Pelanggan cukup mengarahkan kamera ponsel mereka menggunakan aplikasi mobile banking ataupun dompet digital pilihan mereka.

Bagaimana jika pelanggan ingin menggunakan poin reward mereka? Cukup ketikkan jumlah poin loyalitas yang ingin ditukarkan, atau klik tombol Maksimal untuk menukar seluruh poin yang tersedia. Nilai rupiah dari poin tersebut akan otomatis memotong total tagihan pesanan.

Sebelum menekan tombol final, Anda bisa mencentang kotak Kirim Struk WhatsApp. Dengan fitur ini, nota pembayaran digital akan langsung terkirim ke nomor WhatsApp pelanggan tanpa harus membuang-buang kertas struk.

Jika semua rincian sudah benar, klik tombol Buat Pesanan. Dalam hitungan detik, transaksi berhasil diproses! Layar akan memunculkan ringkasan sukses yang jelas menampilkan nomor nota pesanan, nomor meja, total bayar, dan nominal kembalian yang harus diberikan ke pelanggan.

Kasir dapat menekan tombol Cetak Struk untuk mencetak nota ke printer thermal, atau langsung menekan tombol Order Baru untuk langsung mengosongkan keranjang dan melayani antrean pelanggan berikutnya tanpa jeda.
...
Setelah itu, klik tombol Simpan Mutasi. Saldo estimasi di laci kasir otomatis ter-update seketika dan seluruh riwayat mutasi kas ini akan tercatat rapi ke dalam laporan tutup shift nantinya.
...
Kasir memasukkan total uang fisik yang dihitung. Jika ada selisih uang, kasir dapat memberikan alasan atau penjelasan pada kolom catatan selisih. Selanjutnya, klik tombol Hitung dan Tutup Shift.

Seketika itu juga, sistem akan membuka Laporan Rekonsiliasi Kas Lengkap. Mulai dari rincian modal awal saat shift dibuka, total omzet dari penjualan tunai, total uang kas masuk dan kas keluar operasional, total kas teoritis yang seharusnya ada di laci menurut sistem, total kas fisik yang kasir laporkan, serta kalkulasi nilai selisih kas.
