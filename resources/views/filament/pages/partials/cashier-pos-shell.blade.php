...
>
    <div class="cashier-pos">
        ...
        >
            ...
            <div class="cashier-pos-search-row">
                ...
                <button
                    type="button"
                    class="cashier-pos-search-reset"
                    x-show="query.length"
                    x-cloak
                    @click="query = ''"
            ...
            <div class="cashier-pos-chips" role="tablist" aria-label="Kategori menu">
                ...
                @if ($hasUncategorized)
                    <button
                        type="button"
                        class="cashier-pos-chip"
                        :class="category === 'lainnya' && 'is-active'"
                        @click="category = 'lainnya'"
                    >Lainnya</button>
                @endif
            </div>
            ...
            @else
                <div class="cashier-pos-grid">
                    @foreach ($catalog as $item)
                        ...
                        >
                            <div class="cashier-pos-card__photo">
                                @if (filled($item['photo_url'] ?? null))
                                    <img src="{{ $item['photo_url'] }}" alt="" loading="lazy" decoding="async" width="240" height="160">
                                @else
                                    <span>Menu</span>
                                @endif
                            </div>
                            <div class="cashier-pos-card__body">
                                ...
                                <div class="cashier-pos-card__price">
                                    ...
                                    @if ((int) ($item['discount_percent'] ?? 0) > 0)
                                        <span class="cashier-pos-card__was">{{ \App\Support\CmsMedia::formatIdr((int) $item['price']) }}</span>
                                        <span class="cashier-pos-card__discount">-{{ (int) $item['discount_percent'] }}%</span>
                                    @endif
                                </div>
                                @if ($item['has_modifiers'] ?? false)
                                    <button type="button" class="cashier-pos-add" @click="openExtra({{ (int) $item['id'] }})">
                                        Pilih extra
                                    </button>
                                @else
                                    <template x-if="$store.cashierPos.qtyOf({{ (int) $item['id'] }}) > 0">
                                        <div class="cashier-pos-stepper">
                                            <button type="button" @click="bumpItem({{ (int) $item['id'] }}, -1)" aria-label="Kurangi">−</button>
                                            <span x-text="$store.cashierPos.qtyOf({{ (int) $item['id'] }})"></span>
                                            <button type="button" @click="bumpItem({{ (int) $item['id'] }}, 1)" aria-label="Tambah">+</button>
                                        </div>
                                    </template>
                                    <template x-if="$store.cashierPos.qtyOf({{ (int) $item['id'] }}) < 1">
                                        <button type="button" class="cashier-pos-add" @click="addPlain({{ (int) $item['id'] }})">Tambah</button>
                                    </template>
                                @endif
                            </div>
                        </article>
        ...
        <aside class="cashier-pos-summary">
            ...
            >
                ...
                <div class="cashier-pos-field">
                    ...
                    <div class="cashier-pos-combobox">
                        <button
                            type="button"
                            id="cashier-pos-table"
                            class="cashier-pos-select"
                        ...
                        <div
                            class="cashier-pos-combobox__menu"
                            x-show="tableOpen"
                            x-cloak
                            role="listbox"
                            aria-labelledby="cashier-pos-table-label"
                        >
                            <button
                                type="button"
                                class="cashier-pos-combobox__option"
                                :class="tableId === '' && 'is-active'"
                                role="option"
                                @click="pickTable('', 'Pilih meja')"
                            >Pilih meja</button>
                            @foreach ($tables as $id => $code)
                                <button
                                    type="button"
                                    class="cashier-pos-combobox__option"
                                    :class="String(tableId) === '{{ $id }}' && 'is-active'"
                                    role="option"
                                    @click="pickTable('{{ $id }}', {{ \Illuminate\Support\Js::from((string) $code) }})"
                                >{{ $code }}</button>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="cashier-pos-field">
                    ...
                    <input
                        ...
                        x-on:change="onWa($event.target.value)"
                    >
                </div>
                <label class="cashier-pos-check" style="margin-top: 0.7rem" x-show="$store.cashierPos.hasFonnte" x-cloak>
                    <input
                        type="checkbox"
                        x-model="$store.cashierPos.sendReceipt"
            ...
            >
                ...
                <template x-for="line in $store.cashierPos.cartLines" :key="line.index">
                    <div class="cashier-pos-cart-item">
                        ...
                        <div>
                            <p class="cashier-pos-cart-item__name"><span x-text="line.name"></span> <span x-text="'x' + line.qty"></span></p>
                            <p class="cashier-pos-cart-item__meta" x-show="line.extras.length" x-text="line.extras.join(', ')"></p>
                        ...
                        <div>
                            <p class="cashier-pos-cart-item__name" style="text-align: right" x-text="line.total_label"></p>
            ...
            >
                ...
                <div class="cashier-pos-pay">
                    <button
                        type="button"
                        :class="$store.cashierPos.paymentMethod === 'cash' && 'is-active'"
                        @click="setPayment('cash')"
                    >Tunai</button>
                    <button
                        type="button"
                        :class="$store.cashierPos.paymentMethod === 'qris' && 'is-active'"
                        @click="setPayment('qris')"
                    >QRIS</button>
                </div>
            </div>
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between gap-3">
                        <div class="text-sm text-gray-500 dark:text-gray-400">
                            <span x-show="! $store.cashierPos.preview.subtotal">Belum ada item — tambahkan menu untuk melihat perkiraan total.</span>
                            <span x-show="$store.cashierPos.preview.subtotal" x-text="($store.cashierPos.preview.line_count || 0) + ' baris · ' + ($store.cashierPos.preview.total_qty || 0) + ' porsi'"></span>
                        </div>
                        <span
                            ...
                            x-text="$store.cashierPos.paymentMethod === 'qris' ? 'QRIS' : 'Tunai'"
                        ></span>
                    </div>
                    ...
                    <div x-show="$store.cashierPos.preview.subtotal" x-cloak>
                        <dl class="space-y-1.5 text-xs">
                            ...
                            <div class="flex items-baseline justify-between gap-3" x-show="$store.cashierPos.preview.service_amount > 0">
                                <dt class="text-gray-500 dark:text-gray-400">Service (<span x-text="$store.cashierPos.preview.service_pct_label"></span>%)</dt>
                                <dd class="font-medium tabular-nums text-gray-950 dark:text-white" x-text="$store.cashierPos.preview.service_label"></dd>
                            </div>
                            <div class="flex items-baseline justify-between gap-3" x-show="$store.cashierPos.preview.pb1_amount > 0">
                                <dt class="text-gray-500 dark:text-gray-400">PB1 (<span x-text="$store.cashierPos.preview.pb1_pct_label"></span>%)</dt>
                                <dd class="font-medium tabular-nums text-gray-950 dark:text-white" x-text="$store.cashierPos.preview.pb1_label"></dd>
                            </div>
                        </dl>

                        <div class="mt-4 rounded-lg bg-gray-50 p-3 text-center ring-1 ring-gray-950/5 dark:bg-white/5 dark:ring-white/10" x-show="$store.cashierPos.paymentMethod === 'qris'">
                            ...
                            <template x-if="$store.cashierPos.qrisImageUrl">
                                <img :src="$store.cashierPos.qrisImageUrl" alt="QRIS outlet" class="mx-auto block w-full max-w-[11rem] rounded-lg bg-white ring-1 ring-gray-950/5">
                            </template>
                            <p class="text-sm text-gray-500 dark:text-gray-400" x-show="! $store.cashierPos.qrisImageUrl">
                        ...
                        <div class="space-y-2 border-t border-gray-200 pt-4 dark:border-white/10" x-show="$store.cashierPos.paymentMethod === 'cash'" x-cloak>
                            <div class="flex items-center justify-between gap-2">
                                <label for="cashier-pos-cash-received" class="text-sm font-medium text-gray-950 dark:text-white">Uang diterima</label>
                                <button type="button" class="text-sm font-medium text-primary-600 hover:text-primary-500 dark:text-primary-400" @click="cashExact()">Uang pas</button>
                            </div>
                            <div class="flex overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-gray-950/10 dark:bg-white/5 dark:ring-white/20 focus-within:ring-2 focus-within:ring-primary-500">
                                <span class="flex items-center px-3 text-sm text-gray-500 dark:text-gray-400">Rp</span>
                                <input
                                    id="cashier-pos-cash-received"
                                    type="text"
                                    inputmode="numeric"
                                    autocomplete="off"
                                    placeholder="0"
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">Isi nominal uang dari tamu. Kembalian dihitung otomatis.</p>
                           
                            <div x-show="$store.cashierPos.cashParsed() !== null && $store.cashierPos.cashShort()" class="flex items-baseline justify-between gap-3 text-sm">
                                <span class="text-danger-600 dark:text-danger-400">Kembalian</span>
                                <span class="font-bold tabular-nums text-danger-600 dark:text-danger-400" x-text="'Kurang ' + $store.cashierPos.cashFormat(Math.abs($store.cashierPos.cashChange()))"></span>
                            </div>
                            <div x-show="$store.cashierPos.cashParsed() !== null && ! $store.cashierPos.cashShort()" class="flex items-baseline justify-between gap-3 text-sm">
                                <span class="text-gray-500 dark:text-gray-400">Kembalian</span>
                                <span class="font-bold tabular-nums text-success-600 dark:text-success-400" x-text="$store.cashierPos.cashFormat($store.cashierPos.cashChange())"></span>
                            </div>
                        </div>
    ...
    <div
        ...
        x-show="$store.cashierPos.editor"
        x-cloak
        wire:ignore
        role="dialog"
        aria-modal="true"
        aria-labelledby="cashier-pos-modal-title"
    >
        <button type="button" class="cashier-pos-modal__backdrop" @click="$store.cashierPos.closeEditor()" aria-label="Tutup"></button>
        <template x-if="$store.cashierPos.editor">
            <div class="cashier-pos-modal__panel" @click.stop>
                ...
                <textarea id="cashier-pos-notes" class="cashier-pos-textarea" rows="2" x-model="$store.cashierPos.editor.notes" placeholder="Opsional"></textarea>
                <div class="cashier-pos-editor-actions">
                    <button type="button" class="cashier-pos-muted" @click="$store.cashierPos.closeEditor()">Batal</button>
                    <button type="button" class="cashier-pos-add cashier-pos-modal__save" @click="saveEditor()">Simpan</button>
                </div>
            </div>
        </template>
    </div>
