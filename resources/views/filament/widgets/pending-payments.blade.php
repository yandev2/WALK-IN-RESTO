    ...
    <div class="vision-card h-full flex flex-col justify-between p-5 sm:p-6">
        <div>
            ...
            <div class="flex items-center justify-between gap-2 pb-3 border-b border-slate-100 dark:border-white/5">
                ...
                @if ($pendingCount > 0)
                    <span
                        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-extrabold bg-rose-500/15 text-rose-600 dark:text-rose-400 border border-rose-500/20 shadow-sm shrink-0">
                        <span class="w-2 h-2 rounded-full bg-rose-500 animate-pulse"></span>
                        {{ $pendingCount }} Menunggu
                    </span>
                @else
                    <span
                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 shrink-0">
            ...
            <div class="mt-3 divide-y divide-slate-100 dark:divide-white/5">
                @forelse ($orders as $order)
                    ...
                    <div
                        class="py-3 flex items-center justify-between gap-3 group hover:bg-slate-50/50 dark:hover:bg-white/[0.02] -mx-2 px-2 rounded-xl transition-colors">
                        <div class="flex items-center gap-3 min-w-0 flex-1">
                            {{-- Table Avatar Badge --}}
                            <div
                                class="w-10 h-10 rounded-xl bg-gradient-to-br from-sky-600 to-cyan-500 flex items-center justify-center font-extrabold text-white text-xs shrink-0 shadow-sm shadow-sky-500/20">
                                {{ $avatarText }}
                            </div>
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">
                                    ...
                                    · <span
                                        class="uppercase font-semibold text-slate-700 dark:text-slate-300">{{ $order->payment_method ?? 'CASH' }}</span>
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2.5 shrink-0 text-right">
                            ...
                            <span
                                class="w-2.5 h-2.5 rounded-full bg-rose-500 shadow-sm shadow-rose-500/50 animate-pulse"
                                title="Menunggu Kasir"></span>
                        </div>
                    </div>
                @empty
                    <div class="py-8 flex flex-col items-center justify-center text-center">
                        <div
                            class="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-400 mb-2.5">
