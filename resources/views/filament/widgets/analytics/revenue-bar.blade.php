    ...
    <div class="vision-card p-5 sm:p-6 w-full">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-white/5">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-sky-500/10 text-sky-600 dark:text-cyan-400 border border-sky-500/20">
                    <span class="w-2 h-2 rounded-full bg-sky-500 dark:bg-cyan-400"></span>
                    <span>{{ $vision['seven_days_omzet'] }} (7 Hari)</span>
                </span>

                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-purple-500/10 text-purple-600 dark:text-purple-400 border border-purple-500/20">
        ...
        <div
            wire:key="{{ $this->analyticsChartWireKey('revenue-bar') }}"
            class="relative mt-4 w-full"
            x-data="visionAreaChart({ series: @js($vision['series']), categories: @js($vision['categories']) })"
            x-init="init()"
            x-destroy="destroy()"
            wire:ignore
        >
