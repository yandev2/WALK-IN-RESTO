            ...
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
                @foreach ($images as $row)
                    @php
                        ...
                        $src = $row['src'];
                    @endphp
                    <figure class="group relative aspect-4/3 overflow-hidden rounded-3xl bg-surface-muted border border-border-subtle dark:border-zinc-800 shadow-xs hover:shadow-lg transition-all">
                        <button
                            type="button"
                        ...
                        @if ($image->caption)
                            <figcaption class="pointer-events-none absolute inset-x-0 bottom-0 bg-linear-to-t from-black/75 via-black/30 to-transparent px-4 pb-3 pt-8 text-left text-xs font-medium text-white">
                                {{ $image->caption }}
                            </figcaption>
                        @endif
                    </figure>
