    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid items-center gap-12 lg:grid-cols-12 lg:gap-16">
            ...
            <div class="lg:col-span-7">
                ...
                <div class="relative mt-8 rounded-3xl bg-surface-raised dark:bg-zinc-900/90 p-6 sm:p-8 shadow-xl border border-border-subtle dark:border-zinc-800">
                    <span class="absolute -top-4 -left-2 text-6xl font-serif text-primary/30 select-none">“</span>
                    ...
                    <div class="mt-6 pt-5 border-t border-border-subtle dark:border-zinc-800 flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="flex h-11 w-11 items-center justify-center rounded-full bg-linear-to-tr from-primary to-accent text-white font-bold text-base shadow-sm">
                                {{ strtoupper(substr($reviewerName, 0, 1)) }}
                            </div>
                            <div>
                                <h4 class="font-bold text-sm text-body">{{ $reviewerName }}</h4>
                ...
                @if ($restaurant->reviews->count() > 1)
                    <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach ($restaurant->reviews->skip(1)->take(2) as $rev)
                            <div class="rounded-2xl bg-surface-raised dark:bg-zinc-900/90 p-4 border border-border-subtle dark:border-zinc-800 shadow-2xs">
                                <div class="flex items-center justify-between mb-2">
                                    <span class="font-bold text-xs text-body">{{ $rev->displayName() }}</span>
            ...
            <div class="lg:col-span-5 relative flex items-center justify-center">
                <div class="relative w-full max-w-sm mx-auto">
                    {{-- Decorative Backdrop --}}
                    <div class="absolute inset-0 rounded-3xl bg-linear-to-tr from-primary to-accent rotate-3 shadow-2xl opacity-80"></div>
                    ...
                    <div class="relative overflow-hidden rounded-3xl bg-surface-raised dark:bg-zinc-900 p-3 shadow-xl border border-border-subtle dark:border-zinc-800">
                        <img
                            ...
                            class="h-[380px] w-full object-cover rounded-2xl"
                            loading="lazy"
                        >

                        {{-- Floating Satisfaction Badge --}}
                        <div class="absolute bottom-6 right-6 rounded-2xl bg-surface-raised/95 dark:bg-zinc-900/95 backdrop-blur-md p-3.5 shadow-xl border border-border-subtle dark:border-zinc-800 flex items-center gap-3">
