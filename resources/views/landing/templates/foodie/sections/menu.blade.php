    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div x-data="{ activeCategory: 'all' }">
            ...
            @if ($categories->isNotEmpty())
                <div class="mb-10 flex items-center justify-start sm:justify-center gap-2 overflow-x-auto pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    <button
                        type="button"
                        class="shrink-0 rounded-full px-5 py-2 text-xs sm:text-sm font-semibold transition-all"
                        :class="activeCategory === 'all'
                            ? 'bg-primary text-white shadow-md shadow-primary/20 scale-105'
                            : 'bg-surface-raised dark:bg-zinc-900 text-muted hover:text-body border border-border-subtle dark:border-zinc-800'"
        ...
        <div class="mt-12 text-center">
            <a
                href="{{ route('landing.menu', $restaurant) }}"
                class="inline-flex items-center justify-center gap-2 rounded-full border-2 border-primary text-primary hover:bg-primary hover:text-white px-7 py-3 text-sm font-bold shadow-xs transition-all hover:scale-103"
            >
