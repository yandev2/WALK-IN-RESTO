        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="max-w-3xl mx-auto space-y-4">
                @foreach ($restaurant->cmsFaqs as $faq)
                    <details class="group rounded-3xl bg-surface-raised dark:bg-zinc-900 border border-border-subtle dark:border-zinc-800 p-5 sm:p-6 shadow-xs transition-all duration-300 open:border-primary/40 open:shadow-lg">
                        <summary class="flex cursor-pointer items-center justify-between gap-4 font-display font-bold text-base sm:text-lg text-body list-none select-none">
                            <span class="group-hover:text-primary transition-colors">{{ $faq->question }}</span>
                            <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary text-base font-bold transition-all duration-300 group-open:rotate-45 group-open:bg-primary group-open:text-white">
                                +
                            </span>
                        </summary>
