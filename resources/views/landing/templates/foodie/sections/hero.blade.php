        ...
        <div class="grid items-center gap-12 lg:grid-cols-12 lg:gap-8">
            ...
            <div class="lg:col-span-7 flex flex-col items-start">
                ...
                <div class="flex flex-wrap items-center gap-2.5">
                    @if ($outlet)
                        <span class="inline-flex items-center gap-2 rounded-full px-3.5 py-1 text-xs font-semibold hover:scale-105 transition-all duration-300 cursor-default {{ $isOpenNow ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20 shadow-xs' : 'bg-surface-muted text-muted border border-border-subtle' }}">
                    ...
                    @if (filled($copy['pill'] ?? null))
                        <span class="inline-flex items-center gap-1.5 rounded-full bg-linear-to-r from-primary/15 to-accent/15 border border-primary/25 px-3.5 py-1 text-xs font-bold text-primary shadow-2xs hover:scale-105 hover:shadow-sm transition-all duration-300 cursor-default">
                ...
                <h1 class="mt-5 font-display text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-body leading-[1.12]">
                    ...
                    @if (count($words) > 1)
                        {{ $words[0] }}
                        <span class="relative inline-block px-3 py-0.5 mx-1 my-1 rounded-2xl bg-linear-to-r from-primary to-accent text-white shadow-md shadow-primary/20 transform -rotate-1 hover:rotate-0 hover:scale-105 transition-all duration-300 cursor-default">
                            {{ $words[1] }}
                        </span>
                ...
                <div class="mt-8 flex flex-wrap items-center gap-4">
                    @if ($menuItems->isNotEmpty())
                        <a
                            href="#menu"
                            class="inline-flex items-center justify-center gap-2 rounded-full bg-linear-to-r from-primary to-accent hover:opacity-95 px-7 py-3.5 text-sm font-bold text-white shadow-lg shadow-primary/25 hover:shadow-2xl hover:shadow-primary/35 transition-all duration-300 hover:scale-105 active:scale-95 border border-white/20 group"
                        >
                    ...
                    @if ($mapsUrl)
                        <a
                            ...
                            class="inline-flex items-center justify-center gap-2 rounded-full bg-surface-raised dark:bg-zinc-900 border border-border-subtle dark:border-zinc-800 px-6 py-3.5 text-sm font-semibold text-body shadow-xs hover:bg-surface-muted hover:border-primary/40 hover:scale-105 active:scale-95 transition-all duration-300"
                        >
                ...
                @if (($ratingSummary['count'] ?? 0) > 0)
                    <div class="mt-8 flex items-center gap-3 rounded-2xl bg-surface-raised dark:bg-zinc-900/90 border border-border-subtle dark:border-zinc-800 p-3 shadow-xs hover:shadow-lg hover:border-primary/40 hover:scale-102 transition-all duration-300 cursor-default backdrop-blur-xs">
            ...
            <div class="lg:col-span-5 relative flex items-center justify-center group">
                ...
                <div class="absolute inset-0 -m-4 sm:-m-6 rounded-3xl bg-linear-to-br from-primary via-accent to-primary rotate-2 group-hover:rotate-3 group-hover:scale-102 opacity-85 shadow-2xl transition-all duration-500"></div>
                ...
                <div class="relative z-10 w-full overflow-hidden rounded-3xl bg-surface-raised dark:bg-zinc-900 p-2.5 shadow-xl border border-border-subtle dark:border-zinc-800">
                    <img
                        src="{{ $heroImage }}"
                        alt="{{ $restaurant->name }}"
                        class="h-[360px] sm:h-[420px] w-full object-cover rounded-2xl group-hover:scale-105 transition-transform duration-700 ease-out"
                    >

                    {{-- Floating Badge 1: Preparation Time / Service (Top Left) --}}
                    <div class="absolute top-6 left-6 z-20 flex items-center gap-2.5 rounded-full bg-surface-raised/95 dark:bg-zinc-900/95 px-4 py-2 text-xs font-bold text-body shadow-xl border border-border-subtle dark:border-zinc-800 backdrop-blur-xs hover:scale-105 hover:shadow-2xl transition-all duration-300 cursor-default">
                        <span class="flex h-6 w-6 items-center justify-center rounded-full bg-linear-to-br from-primary/20 to-accent/20 text-primary">
                    ...
                    @if ($outlet)
                        <div class="absolute bottom-6 right-6 z-20 flex items-center gap-2.5 rounded-2xl bg-surface-raised/95 dark:bg-zinc-900/95 p-3 text-xs font-semibold text-body shadow-xl border border-border-subtle dark:border-zinc-800 backdrop-blur-xs hover:scale-105 hover:shadow-2xl transition-all duration-300 cursor-default">
                            <div class="flex h-8 w-8 items-center justify-center rounded-full bg-linear-to-br from-primary/20 to-accent/20 text-primary">
