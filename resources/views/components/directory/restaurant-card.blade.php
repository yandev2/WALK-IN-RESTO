...
<a
    ...
    @class([
        'directory-card group relative overflow-hidden rounded-[1.35rem] bg-surface-raised ring-1 ring-[color:var(--border-subtle)] shadow-sm hover:shadow-md transition-all duration-300',
        'directory-card-grid' => $variant === 'grid',
    ])
>
    <div class="directory-card-media relative overflow-hidden bg-surface-muted">
        @if ($card['cover_url'])
            <img src="{{ $card['cover_url'] }}" alt="" class="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy">
        @else
        ...
        @if ($card['price_label'])
            <div class="absolute right-2.5 top-2.5 z-10">
                <span class="inline-flex rounded-full bg-black/60 px-2.5 py-0.5 text-[11px] font-bold text-white backdrop-blur-md shadow-sm">
                    {{ $card['price_label'] }}
                </span>
            </div>
        @endif
            ...
            <div class="flex items-start justify-between gap-2">
                <div class="min-w-0 flex-1">
                    @if ($card['categories_label'])
                        <p class="truncate text-xs font-semibold text-primary mb-1">{{ $card['categories_label'] }}</p>
                    @endif
                    <h3 class="truncate font-display text-base font-bold text-body sm:text-lg group-hover:text-primary transition-colors duration-200">{{ $card['name'] }}</h3>
                </div>
            </div>

            @if ($card['headline'])
                <p class="mt-1.5 line-clamp-2 text-xs sm:text-sm leading-relaxed text-muted">{{ $card['headline'] }}</p>
            @endif
        </div>
        ...
        <div class="mt-3 flex items-center justify-between border-t border-border-subtle/60 pt-3 text-xs sm:text-sm">
            <div class="flex items-center gap-2">
                @if ($card['rating_average'])
                    <span class="inline-flex items-center gap-1 font-bold text-body">
            ...
            <div class="flex items-center gap-2">
                ...
                <span class="hidden items-center text-xs font-bold text-primary group-hover:translate-x-1 transition-transform sm:inline-flex">
                    Buka →
                </span>
            </div>
        </div>
